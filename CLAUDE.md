...
2. **Python MCP server** (`graph_mcp_driver/`) — exposes Neo4j Cypher and Fuseki SPARQL queries to an MCP-capable LLM host via the Model Context Protocol, enabling the LLM to query the graph.
...
The zip (`target/msosa-model-exporter-2.0.3-Preview-plugin.zip`) extracts to a folder you drop into `<MSOSA_HOME>/plugins/`.
...
Credentials come from environment variables; defaults match the Docker setup. The MCP host loads the server over stdio; a sample host config lives at `Claude_Desktop_Config/claude_desktop_config.json`.
...
| `ontology/queries/semantic-search-examples.sparql` | Anchor SPARQL queries that ground the LLM's semantic-search answers. |
...
2. Unzip `target/msosa-model-exporter-2.0.3-Preview-plugin.zip` into `<MSOSA_HOME>/plugins/`
3. Restart MSOSA
