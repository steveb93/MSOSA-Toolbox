...
2. Select `target/msosa-model-exporter-2.0.6-Preview-plugin.zip`
...
└── plugins/com.uaf.neo4j.plugin/
    ...
    └── msosa-model-exporter-2.0.6-Preview.jar
```

Unzip `target/msosa-model-exporter-2.0.6-Preview-plugin.zip` directly into `<MSOSA_HOME>`. Restart MSOSA. The plugin appears under **Tools → UAF Neo4j Export**.
