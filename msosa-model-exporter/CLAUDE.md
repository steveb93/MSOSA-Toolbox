...
Outputs:
- `target/msosa-model-exporter-1.4.1-Preview.jar` — fat jar (Neo4j driver bundled + relocated)
- `target/msosa-model-exporter-1.4.1-Preview-plugin.zip` — drop into `<MSOSA_HOME>/plugins/`
...
<MSOSA_HOME>/plugins/msosa-model-exporter/
    msosa-model-exporter-1.4.1-Preview.jar
    plugin.xml
