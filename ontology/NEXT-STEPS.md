...
- [ ] Pick a triplestore. **GraphDB is excluded** by current work-environment policy (Free/SE/EE — local install used only for evaluation). Realistic options: stay on Fuseki and upgrade the reasoner profile from RDFS-Exp to OWL Mini or OWL FB (cheapest — keeps stack at one engine; OWL FB covers property chains); or move to Eclipse RDF4J Server (Apache/EPL-licensed, OWL 2 RL via SPIN). Stardog Community has a 10M-triple cap and licence terms that need explicit workplace approval. **Current recommendation: stay on Fuseki + OWL FB**; revisit only if DL-complete entailment becomes mandatory.
