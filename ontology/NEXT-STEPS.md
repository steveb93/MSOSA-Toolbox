...
- [x] **OWL 2 axioms.** Authored as `ontology/uaf-mvo-axioms.ttl`, sibling to the generated MVO. Covers:
  ...
  - `uaf:dominates` as `owl:TransitiveProperty` for dominance between UAF SecurityElements (architectural Security domain). Data-level classification dominance — the TS/S/C/U lattice the DoD Data Marking Plugin applies to ERD Entities — sits in UML, not UAF, and is layered separately via `ontology/uml-data-marking.ttl` (`dm:` namespace, `dm:dominates` property, four named individuals, JOIN via `skos:notation` against `uaftv:securityClassification` literals). `uaf:dominates` is retained as a paired example; no UAF stereotype emits DOMINATES today.
