...
- [x] **Self-hosted WebVOWL container** — `docker-compose.fuseki.yml` runs a `webvowl` service alongside Fuseki, built from the upstream repo at <https://github.com/VisualDataWeb/WebVOWL> per its README ("`docker build . -t webvowl:v1`"). No published image exists; `WEBVOWL_REF` in `docker-compose/.env` pins the git ref (default `master`). UI binds to `http://localhost:${WEBVOWL_PORT:-8080}/`. T-Box is loaded by uploading `ontology/uaf-mvo.ttl` (and optionally `uaf-mvo-axioms.ttl`) through the WebVOWL UI. Replaces the earlier SPARQL→GraphML exporter — fewer intermediary steps, no extra Python deps for the viewer path.
...
| **Static GraphML exporter withdrawn — self-hosted WebVOWL only** | 2026-05-26 | Reversed the 2026-05-24 decision after use revealed the SPARQL→GraphML→desktop-tool workflow had too many intermediary steps to be worth keeping (run `dump_to_rdf` → run `sparql_to_graphml` → save file → open in Cytoscape/yEd/Gephi). Deleted `ontology/codegen/sparql_to_graphml.py`, the four `ontology/visualisations/queries/*.sparql` presets, the visualisations README, and `Test/test_graphml_export.py`. T-Box viewing now runs through a WebVOWL container added to `docker-compose.fuseki.yml`, built from the upstream repo at <https://github.com/VisualDataWeb/WebVOWL> on first `up` per the upstream README (there is no published image; `WEBVOWL_REF` in `.env` pins the git ref). One URL, no intermediary file, stays alongside Fuseki. No A-Box viewer is shipped by default; re-open if concrete demand reappears. |
