...
- [x] **Relationship-stereotype map** for `OperationalExchange`, `ResourceInteraction`, `NeedLine` (and Tier-1 real-world profile additions: `Implements`, `IsCapableToPerform`, `PerformsInContext`, `MapsToCapability`, `DataAssociation`, the four `*Association` family stereotypes, `Allocate`, `DeriveReqt`, `Copy`, `SequenceFlow`, `MessageFlow`). PRs #78 + #82.
...
| **Post canonical relation is `partOf Organization`, not `heldBy Person`** | 2026-05-24 | The Stage-3 D1 NEXT-STEPS bullet sketched `Post ⊑ ∃ heldBy . Person`. On checking the registered class set for D2, UAF has no first-class `Person` — Post itself is the canonical role-bearing element, with `partOf Organization` (using `uaf:partOf`, inverse of `uaf:composedOf`) as the definitional relation. Shaped accordingly in the D2 PR. ActualOrganization is the realisation of an Organization, but the Person/role-incumbent layer some MBSE traditions assume is not part of UAF's domain model as registered. |
| **Stage 5 graph algorithms via NetworkX bridge, not Neo4j GDS** | 2026-05-24 | User confirmed graph algorithms (PageRank, centrality, community detection) are wanted *eventually*. Without an RDF-native alternative, Stage 5 would force Neo4j to stay forever — blocking the agreed one-triplestore end-state. Direction: SPARQL → GraphML (already shipped via `sparql_to_graphml.py` in PR #104) → NetworkX/igraph in Python → results written back to Fuseki as `uafprop:*` annotations. UAF models are small enough (thousands of nodes) that NetworkX is adequate. Defer building the bridge until Stage 5 demand arrives — keep Neo4j running until then, but progressively reduce dependence so the cutover is cheap. Stage 5 section below to be updated when the bridge actually lands. |
| **Stage 3 whole-ontology complete** | 2026-05-24 | Closing decision for Stage 3's SHACL + OWL strand. After D1 (Operational + Resource) and D2 (Service + Personnel + Acquisition) landed, governance constraints span all 7 UAF domains: 24 SHACL NodeShapes + 12 `owl:someValuesFrom` restrictions + 16 `owl:inverseOf` pairs + pairwise disjointness + `uaf:dominates` transitivity. Remaining Stage 3 items (outbound SERVICE federation, ExportSummaryDialog SHACL row) are independent of domain coverage and can land or defer on their own cadence. This is a natural quality bar for the `preview` → `main` merge candidate. |
