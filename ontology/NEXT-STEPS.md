...
- [x] **Self-hosted WebVOWL container** — `docker-compose.fuseki.yml` runs a `webvowl` service alongside Fuseki. Image is parameterised via `WEBVOWL_IMAGE` in `docker-compose/.env`; UI binds to `http://localhost:${WEBVOWL_PORT:-8080}/`. T-Box is loaded by uploading `ontology/uaf-mvo.ttl` (and optionally `uaf-mvo-axioms.ttl`) through the WebVOWL UI. Replaces the earlier SPARQL→GraphML exporter — fewer intermediary steps, no extra Python deps for the viewer path.
