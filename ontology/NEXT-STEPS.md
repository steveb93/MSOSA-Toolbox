...
- [x] **Extend the Java plugin's `ExportSummaryDialog` with a SHACL conformance row** — in-process via shaded `jena-shacl:4.10.0`, runs on the in-memory RDF model under an OWL FB reasoner so verdicts match the standalone `validate_shacl.py` validator. Bundled ontology resources (MVO + axioms + shapes) ride on the classpath at `/uaf-ontology/...` via `maven-resources-plugin`. `ExportResult` carries `shaclConformance`/`shaclViolations`/`shaclWarnings`/`shaclViolationLines`; the dialog shows a `"SHACL conformance"` row ("Pass" / "Pass (N warnings)" / "Fail (N violations)" / "N/A (RDF not enabled)") and a `"SHACL Violations (N)"` tab when entries exist. Reasoner-introduced blank-node existentials are filtered out — only URI-named instances under `uafinst:` are reported. 3 new unit tests at `ShaclValidationServiceTest`.
