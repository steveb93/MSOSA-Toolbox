...
- [x] **Self-hosted WebVOWL container** — `docker-compose.fuseki.yml` runs a `webvowl` service alongside Fuseki, built from source via `docker-compose/webvowl/Dockerfile`. The upstream image (https://github.com/VisualDataWeb/WebVOWL) downloads a pre-built WAR from a host that no longer serves it, so we rebuild from the upstream git source ourselves (multi-stage: Node 8 alpine build → nginx alpine serve). `WEBVOWL_REF` in `docker-compose/.env` pins the upstream git ref (default `master`). UI binds to `http://localhost:${WEBVOWL_PORT:-8080}/`. T-Box is loaded by uploading `ontology/uaf-mvo.ttl` (and optionally `uaf-mvo-axioms.ttl`) through the WebVOWL UI. Replaces the earlier SPARQL→GraphML exporter — fewer intermediary steps, no extra Python deps for the viewer path.
...
| **Static GraphML exporter withdrawn — self-hosted WebVOWL only** | 2026-05-26 | Reversed the 2026-05-24 decision after use revealed the SPARQL→GraphML→desktop-tool workflow had too many intermediary steps to be worth keeping (run `dump_to_rdf` → run `sparql_to_graphml` → save file → open in Cytoscape/yEd/Gephi). Deleted `ontology/codegen/sparql_to_graphml.py`, the four `ontology/visualisations/queries/*.sparql` presets, the visualisations README, and `Test/test_graphml_export.py`. T-Box viewing now runs through a WebVOWL container added to `docker-compose.fuseki.yml`, built from source via `docker-compose/webvowl/Dockerfile` (multi-stage Node 8 alpine → nginx alpine; the upstream pre-built WAR host is dead so we rebuild from the upstream git source instead). `WEBVOWL_REF` in `.env` pins the upstream git ref. One URL, no intermediary file, stays alongside Fuseki. No A-Box viewer is shipped by default; re-open if concrete demand reappears. |
